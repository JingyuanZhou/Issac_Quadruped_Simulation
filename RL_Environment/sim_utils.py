...
def load_asset(gym, sim, robot, fix_base_link):
    if robot is RobotType.ALIENGO:
        ...
        asset_file = ANYMAL
    elif robot is RobotType.CHEETAH:
        asset_file = CHEETAH
    else:
...
def add_terrain(gym, sim, name="slope", x_offset=2., invert=False, width=2.8):
    ...
    if name=="slope":
        ...
        stone_distance=0.3
        max_height=3
        heightfield[0: num_rows, :] = stepping_stones_terrain(new_sub_terrain(), stone_size=stone_size, stone_distance=stone_distance,max_height=max_height).height_field_raw
    else:
